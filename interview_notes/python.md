...
Управление памятью в CPython основано на **reference counting**, 
а **cyclic gc** используется как дополнительный механизм для удаления циклических ссылок.

### Reference counting
Каждый объект рантайма **_CPython_** (_PyObject_) имеет **атрибут** `ref_count` – **счетчик ссылок**. 
Когда он становится равным 0, объект сразу **деаллоцируется**. 

**Оператор** `del` **декрементирует счетчик ссылок** и НЕ удаляет объект. Еще
он ломает сборку циклических объектов, т.к. неясно в каком порядке лучше вызывать del.

Сборщик мусора работает с **тремя поколениями объектов**. 
Изначально объект принадлежит **первому поколению**, 
но далее он может перейти в следующее, если переживет сборку.

**GC запускается**, когда разница между количеством аллокаций и деаллокаций 
превышает заданный threshold (порог). Пороги запуска можно переопределить с помощью метода `gc.set_threshold`.

**При отключенном GC** все обычные объекты будут удаляться нормально, 
но циклические ссылки будут оставаться и создавать утечки памяти.


### Cyclic GC

Механизм работы с циклическими ссылками работает только с контейнерными объектами
(list, dict, пользовательские объекты), временно вычитая внутренние ссылки (см. ниже)
...
def f():
    ...
    # внутри функции:
    # a.ref_count == 2 (b внутри a - одна внутренняя ссылка и переменная a - одна внешняя ссылка)
    # b.ref_count == 2 (a внутри b - одна внутренняя ссылка и переменная b - одна внешняя ссылка)
    
    # вычитаем внутренние ссылки:
    # a.ref_count - 1 == 1 и b.ref_count - 1 == 1
    # есть внешние ссылки - НЕЛЬЗЯ УДАЛИТЬ

# после выхода из функции f:
# a.ref_count == 1 (b внутри a - одна внутренняя ссылка, переменная a уничтожена и внешних ссылок нет)
# b.ref_count == 1 (a внутри b - одна внутренняя ссылка, переменная b уничтожена и внешних ссылок нет)

# вычитаем внутренние ссылки:
# a.ref_count - 1 == 0 и b.ref_count - 1 == 0 -> внешних ссылок нет -> МОЖНО УДАЛИТЬ
...
**Stack frame функции** — объект памяти в куче для хранения локальных переменных, аргументов, текущая позиция выполнения.

Обычная функция при _return_ завершает работу и её stack frame уничтожается,
поэтому при новом вызове всё начинается с нуля. 

В генераторе при _yield_ stack frame НЕ удаляется, из-за чего контекст выполнения генератора остается прежним.
