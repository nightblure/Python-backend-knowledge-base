...
## Протокол итератора/как работает цикл for

**Итератор** - объект с реализацией метода `__next__`, содержащего логику перебора некой последовательности.

**Итерируемый объект** - объект с реализацией метода `__iter__` ИЛИ метода `__getitem__`.

Для возможности использования в цикле объект должен реализовать 
метод или методы одним из следующих способов:
* только `__getitem__`;
* `__iter__` и `__next__`;
* только `__iter__`, если он возвращает итерируемый объект.

Цикл `for` пытается взять итератор от объекта (`iter(obj)`) и вызывать его метод `__next__`, 
...
Метод/функция должна зависеть от абстракции, 
а внедряемая зависимость должна реализовывать интерфейс абстракции:
```python
