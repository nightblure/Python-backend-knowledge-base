...
Защищает от аномалии "грязного чтения", но не защищает от "неповторяемого чтения".

Попытка изменения одних и тех же данных в *T2* до коммита изменений *T1* будет заблокирована. 
***Является уровнем по умолчанию в *PostgreSQL***.

***L3. REPEATABLE READ*** – *T1* увидит изменения, выполненные *T2*, только после коммита *T2* и *T1*.
