...
PG работает в режиме **Multi-Version Concurrency Control** (_MVCC_),
что дает параллелизм при чтении и записи в разных транзакциях.

**При обновлении** строка не изменяется на месте, но создаётся новая версия,
а старая остаётся доступной для других транзакций.

**Оптимическая блокировка** реализуется так: при обновлении версия инкрементируется и **создается новая строка**:
```sql
UPDATE table
...
Если никто не изменил строку → update сработает. На это и есть расчет по умолчанию в MVCC.
Но защититься явным образом можно с помощью явных (пессимистических) блокировок и уровней изолированности транзакций.
...
***L1. READ UNCOMMITTED*** – **T1** **видит изменения**, выполненные **незавершенной** **T2**. 
Этот уровень не реализован в *PostgreSQL*, т.к. не представляет практической ценности.


***L2. READ COMMITTED*** – *T1* видит изменения, выполненные *T2*, после коммита *T2*.

Защищает от аномалии "грязного чтения", но не защищает от "**неповторяемого чтения**". Является уровнем 
по умолчанию в **PostgreSQL**.

**Неповторяемое чтение** - получение разных данных внутри одной транзакции:
```
T1: BEGIN
...
T1: SELECT * FROM table -- получит измененные данные - неповторяемое чтение
...
```


***L3. REPEATABLE READ*** – *T1* увидит изменения, выполненные *T2*, только после коммита **обеих транзакций**.

Защищает от аномалии "неповторяемого чтения", но не защищает от "фантомного чтения".

Фантомное чтение может возникнуть тогда, когда Т1 внутри себя не увидит строк, которые успела создать Т2.

Попытка изменения одних и тех же данных в *T2* до коммита изменения в *T1* будет заблокирована.


***L4. SERIALIZABLE*** – уровень, на котором моделируется последовательное выполнение всех зафиксированных транзакций.
Если транзакции пытаются менять пересекающийся набор данных, то упадет ошибка 
`could not serialize access due to concurrent update`, которую нужно отлавливать и ретраить.

Шаблон для наглядного представления и работы с уровнями изоляции можно посмотреть в файле `isolation_levels_guide.sql`.
...
**Мастер-слейв репликация** предполагает главного (мастер) сервера и подчиненного (слейв) сервера. 
**Слейв повторяет состояние мастера** и не может менять данные самостоятельно.

**Репликация** **мастер-мастер предполагает равенство серверов**. 
Оба сервера могут обрабатывать запросы на чтение и изменение данных.
...
**Физическая** (**потоковая**) репликация – вид репликации,
в основе которого лежит **передача записей журнала транзакций** (_WAL_) от мастера к реплике.
**Реплика применяет изменения**, принятые от мастера, **чисто механически**, 
без «понимания смысла», работая **в режиме постоянного восстановления**. 
Поэтому данный вид репликации можно применить только в условиях полной 
программно-аппаратной совместимости мастер и слейв серверов.
...
В основе **логической** репликации лежит принцип ***publisher*-*subscriber***, 
при этом у серверов нет выделенных ролей: **один и тот же сервер может публиковать изменения и быть подписчиком**.

Сервер-паблишер читает журнальные записи и предварительно декодирует в **логический**, **платформо-независимый** вид. 
Поэтому логическая репликация не требует жесткой совместимости серверов, реплика должна лишь понимать протокол репликации.

**Логическая репликация не реплицирует** команды *DDL* (все изменения схемы данных нужно переносить вручную), 
...
Денормализация обычно проводится путем добавления данных, которые согласно требования 
нормальных форм должны выноситься в отдельную таблицу (избыточность данных).
